...
namespace BackgroundService.Source.Services.System.API.VirtualDesktop
{
    internal class VirtualDesktopAPIW11
    {
		...
		{
			private List<object> comResources = new List<object>();
			...
			public List<IApplicationView> GetApplicationViews()
			{
			...
			public List<IVirtualDesktop> GetVirtualDesktops()
			{
				var comDesktops = GetResource(() =>
				{
					DesktopManager.VirtualDesktopManagerInternal.GetDesktops(IntPtr.Zero, out IObjectArray _comDesktops);
					return _comDesktops;
				});
			...
			public IVirtualDesktop GetCurrentVirtualDesktop()
			{
				return DesktopManager.VirtualDesktopManagerInternal.GetCurrentDesktop(IntPtr.Zero);
			}

			private List<T> CastResourceTo<T>(IObjectArray arr)
			{
			...
			private T GetResource<T>(Func<T> getter)
			{
				var resource = getter();
				if (resource != null)
				{
		...
		internal static class DesktopManager
		{
			...
			internal static IApplicationView GetApplicationView(IntPtr hWnd)
			...
			{ // move window to this desktop
				...
				{ // window of other process
					...
					{ // could not move active window, try main window (or whatever windows thinks is the main window)
						DesktopManager.ApplicationViewCollection.GetViewForHwnd(Process.GetProcessById(processId).MainWindowHandle, out view);
						DesktopManager.VirtualDesktopManagerInternal.MoveViewToDesktop(view, ivd);
					}
				}
			}
